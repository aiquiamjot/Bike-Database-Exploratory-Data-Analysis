...
-- list top 10 brands by number of sales

WITH product_quantities AS (
SELECT oi.product_id, quantity, p.brand_id, brand_name
FROM order_items oi
JOIN products p
...
SELECT brand_name, SUM(quantity) as total_quantity_sales
...
-- list top 10 brands by revenue

WITH order_products_id AS (
SELECT oi.product_id, quantity, oi.list_price, discount, p.brand_id, brand_name
FROM order_items oi
JOIN products p
	ON oi.product_id = p.product_id
JOIN brands b
...
SELECT customer_status, COUNT(customer_status) as amount
...
-- top 10 brands with the most total discount (in USD). 
...
SELECT o.order_id, SUBSTRING(order_date, 1, 4) AS `year`, quantity, list_price, discount, store_name
FROM orders o
JOIN order_items oi
...
SELECT oi.product_id, oi.quantity, oi.list_price, discount, 
